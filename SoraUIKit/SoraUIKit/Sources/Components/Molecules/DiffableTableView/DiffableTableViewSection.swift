...
open class DiffableTableViewSection: DiffableTableViewSectionProtocol {
    private var uuid = UUID()

    public var rows: [DiffableTableViewItemProtocol] {
        didSet {
    ...
    public var header: DiffableTableViewItemProtocol? {
        didSet {
    ...
    public init(rows: [DiffableTableViewItemProtocol] = []) {
        self.rows = rows
    }
    ...
    public func heightForHeader(in section: Int, in tableView: DiffableTableView) -> CGFloat {
        return header?.itemHeight(forWidth: tableView.frame.size.width, context: context) ?? 0
    }

    public func cellForRow(at indexPath: IndexPath, in tableView: DiffableTableView) -> UITableViewCell {
        let item = rows[indexPath.row]
        let cell = tableView.sora.dequeueCell(for: indexPath)
        (cell as? DiffableTableViewCellProtocol)?.set(item: item, context: context)
        return cell
    }

    public func viewForHeader(in tableView: DiffableTableView) -> UIView? {
        ...
        (headerView as? DiffableTableViewCellProtocol)?.set(item: header, context: context)
        return headerView
    }
...
extension DiffableTableViewSection: Hashable {
    public static func == (lhs: DiffableTableViewSection, rhs: DiffableTableViewSection) -> Bool {
